...
function ServiceAccountRow({ account, orgs, onChanged, onRevealToken }) {
  ...
  async function saveOrgs() {
    setBusy(true);
    try {
      await api.patch(`/api/security/service-accounts/${account.id}/`, { org_slugs: selected });
      setEditing(false);
      onChanged();
    } finally {
      setBusy(false);
        ...
        {editing ? (
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Button size="sm" disabled={busy} onClick={saveOrgs} className="text-xs">Save</Button>
              <Button
                size="sm"
                variant="ghost"
                disabled={busy}
                onClick={() => { setSelected(account.orgs.map(o => o.slug)); setEditing(false); }}
                className="text-xs"
              >
                Cancel
              </Button>
